"""Load FitPrint fitness reports from MetSights.

For participants in running engagements where assessment_instance.status == 'completed',
today >= engagement_date, and the package is FitPrint Full (type 7):
1. Fetch report detail from ``/reports/fitness-reports/:record_id/``.
2. Fetch PDF URL from ``/reports/fitness-reports/:record_id/pdf/``.
3. Persist on ``individual_health_report`` for the assessment instance.

These are fitness reports — not MetSights BioAI / metabolic health reports.
"""

from __future__ import annotations

import logging
from datetime import date
from typing import Any

from sqlalchemy import select
from sqlalchemy.ext.asyncio import AsyncSession

from core.config import settings
from modules.assessments.models import AssessmentInstance, AssessmentPackage
from modules.audit.cron_sync_logging import tracked_integration_call
from modules.engagements.models import Engagement, EngagementParticipant
from modules.metsights.service import MetsightsService
from modules.notifications.load_bioai_reports import (
    _extract_report_file_url,
    _report_data_complete,
)
from modules.notifications.load_blood_reports import _get_or_create_ihr
from modules.reports.repository import ReportsRepository

logger = logging.getLogger(__name__)

_FITPRINT_TYPE_CODE = "7"


def _fitprint_report_url(*, record_id: str, pdf: bool = False) -> str:
    base = settings.METSIGHTS_BASE_URL.rstrip("/")
    suffix = "/pdf/" if pdf else "/"
    return f"{base}/reports/fitness-reports/{record_id}{suffix}"


async def _get_eligible_participants(
    db: AsyncSession,
    today: date,
    *,
    all_engagements: bool = False,
) -> list[tuple]:
    canonical_ihr = ReportsRepository.canonical_individual_health_report_subquery()
    query = (
        select(
            EngagementParticipant.user_id,
            Engagement.engagement_id,
            AssessmentInstance.metsights_record_id,
            AssessmentPackage.assessment_type_code,
            canonical_ihr.c.reports,
            canonical_ihr.c.report_url,
            canonical_ihr.c.report_id,
            AssessmentInstance.assessment_instance_id,
        )
        .join(Engagement, Engagement.engagement_id == EngagementParticipant.engagement_id)
        .join(
            AssessmentInstance,
            (AssessmentInstance.engagement_id == EngagementParticipant.engagement_id)
            & (AssessmentInstance.user_id == EngagementParticipant.user_id),
        )
        .join(AssessmentPackage, AssessmentPackage.package_id == AssessmentInstance.package_id)
        .outerjoin(
            canonical_ihr,
            canonical_ihr.c.assessment_instance_id == AssessmentInstance.assessment_instance_id,
        )
        .where(AssessmentInstance.status == "completed")
        .where(EngagementParticipant.engagement_date <= today)
        .where(AssessmentInstance.metsights_record_id.isnot(None))
        .where(AssessmentInstance.metsights_record_id != "")
        .where(AssessmentPackage.assessment_type_code == _FITPRINT_TYPE_CODE)
    )
    if not all_engagements:
        query = query.where(Engagement.status.ilike("running"))
    result = await db.execute(query)
    return result.all()


async def load_fitprint_reports(
    db: AsyncSession,
    *,
    metsights_service: MetsightsService,
    as_of: date | None = None,
    dry_run: bool = False,
    all_engagements: bool = False,
) -> dict[str, Any]:
    today = as_of or date.today()
    participants = await _get_eligible_participants(
        db,
        today,
        all_engagements=all_engagements,
    )
    matched = len(participants)
    loaded = 0
    skipped = 0
    failed = 0
    details: list[dict[str, Any]] = []

    for row in participants:
        (
            user_id,
            engagement_id,
            record_id,
            type_code,
            existing_reports,
            existing_report_url,
            ihr_id,
            instance_id,
        ) = row

        record_id = (record_id or "").strip()
        if not record_id:
            skipped += 1
            details.append({
                "user_id": user_id,
                "engagement_id": engagement_id,
                "action": "skipped",
                "reason": "no metsights_record_id",
            })
            continue

        if dry_run:
            complete = _report_data_complete(existing_reports, existing_report_url)
            details.append({
                "user_id": user_id,
                "engagement_id": engagement_id,
                "action": "dry_run",
                "reason": "reports_complete" if complete else "reports_incomplete",
            })
            continue

        try:
            reports = existing_reports
            report_url = existing_report_url

            if not _report_data_complete(reports, report_url):
                fetched_reports = None
                fetched_url = None

                if reports is None:
                    report_data = await tracked_integration_call(
                        db,
                        provider="metsights",
                        api_url=_fitprint_report_url(record_id=record_id),
                        engagement_id=engagement_id,
                        user_id=user_id,
                        request_payload={"record_id": record_id, "assessment_type_code": type_code},
                        operation=lambda: metsights_service.get_report(
                            record_id=record_id,
                            assessment_type_code=type_code,
                        ),
                        reraise=False,
                    )
                    if report_data is not None and report_data:
                        fetched_reports = report_data
                        if fetched_url is None:
                            fetched_url = _extract_report_file_url(report_data)
                    elif report_data is None:
                        logger.warning(
                            "MetSights FitPrint get_report failed for record=%s",
                            record_id,
                        )

                if report_url is None:
                    pdf_data = await tracked_integration_call(
                        db,
                        provider="metsights",
                        api_url=_fitprint_report_url(record_id=record_id, pdf=True),
                        engagement_id=engagement_id,
                        user_id=user_id,
                        request_payload={"record_id": record_id, "assessment_type_code": type_code},
                        operation=lambda: metsights_service.get_report_pdf(
                            record_id=record_id,
                            assessment_type_code=type_code,
                        ),
                        reraise=False,
                    )
                    if pdf_data is not None:
                        file_url = pdf_data.get("file") or pdf_data.get("url")
                        if file_url:
                            fetched_url = file_url
                    else:
                        logger.warning(
                            "MetSights FitPrint get_report_pdf failed for record=%s",
                            record_id,
                        )

                if fetched_reports is None and fetched_url is None:
                    skipped += 1
                    details.append({
                        "user_id": user_id,
                        "engagement_id": engagement_id,
                        "action": "skipped",
                        "reason": "no FitPrint report data returned from MetSights",
                    })
                    continue

                ihr = await _get_or_create_ihr(
                    db,
                    ihr_id=ihr_id,
                    user_id=user_id,
                    engagement_id=engagement_id,
                    instance_id=instance_id,
                )

                if fetched_reports is not None:
                    ihr.reports = fetched_reports
                    reports = fetched_reports
                if fetched_url is not None:
                    ihr.report_url = fetched_url
                    report_url = fetched_url
                await db.flush()
                await db.commit()

                if not _report_data_complete(reports, report_url):
                    skipped += 1
                    details.append({
                        "user_id": user_id,
                        "engagement_id": engagement_id,
                        "action": "skipped",
                        "reason": "FitPrint report data incomplete (missing reports or report_url)",
                    })
                    continue

                loaded += 1
                details.append({
                    "user_id": user_id,
                    "engagement_id": engagement_id,
                    "action": "loaded",
                    "reason": "FitPrint report data fetched from MetSights",
                })
            else:
                skipped += 1
                details.append({
                    "user_id": user_id,
                    "engagement_id": engagement_id,
                    "action": "skipped",
                    "reason": "FitPrint report already complete",
                })

        except Exception as exc:
            await db.rollback()
            failed += 1
            details.append({
                "user_id": user_id,
                "engagement_id": engagement_id,
                "action": "failed",
                "reason": str(exc)[:200],
            })
            logger.warning(
                "load_fitprint_reports failed: user=%s engagement=%s: %s",
                user_id,
                engagement_id,
                exc,
                exc_info=True,
            )

    return {
        "as_of": today.isoformat(),
        "matched": matched,
        "loaded": loaded,
        "skipped": skipped,
        "failed": failed,
        "dry_run": dry_run,
        "details": details,
    }
