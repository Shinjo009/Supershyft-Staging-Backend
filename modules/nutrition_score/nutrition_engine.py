...
def _to_questionnaire_lookup(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    height and waist as ``{"value": ..., "unit": ...}``. Scalar values are
    wrapped here only. ``height_unit`` is accepted as a compatibility alias.
    """
    lookup = dict(payload)
    ...
    waist = lookup.get("waist_circumference")
    if waist is None:
        waist = lookup.get("waist")
    if waist is not None and not isinstance(waist, dict):
        lookup["waist_circumference"] = {
            "value": waist,
            "unit": lookup.get("waist_unit"),
        }
    return lookup
