...
@dataclass(frozen=True)
class MacroTargetSet:
    """Healthy-band macro / fibre TARGET ranges for one goal (or general)."""
...
@dataclass(frozen=True)
class TargetGeneratorConfig:
    """Extra Target Generator configuration from targets.yaml (Phase 11)."""

    goal_macro_targets: dict[GoalId, MacroTargetSet]
    general_macro_targets: MacroTargetSet
    water_targets: dict[str, dict[str, TargetRange]]
    activity_water_uplift_l: dict[str, float]
    macro_combination: dict[str, str]
    zero_goal_target_keys: tuple[str, ...]


@dataclass(frozen=True)
class BodyMetricTargetConfig:
    """Product-configured body metric references and goal overrides."""

    bmr_reference_ranges: dict[str, dict[str, TargetRange]]
    waist_general: dict[str, TargetRange]
    waist_goal_overrides: dict[GoalId, dict[str, TargetRange]]
    body_fat_general: dict[str, TargetRange]
    body_fat_goal_overrides: dict[GoalId, dict[str, TargetRange]]
...
class NutritionEngineConfig:
    ...
    config_dir: Path
    target_generator: TargetGeneratorConfig | None = None
    body_metric_targets: BodyMetricTargetConfig | None = None
...
def load_nutrition_engine_config(config_dir: Path | None = None) -> NutritionEngineConfig:
    ...
    target_generator = _parse_target_generator(raw["targets.yaml"], targets=targets)
    body_metric_targets = _parse_body_metric_targets(raw["targets.yaml"])
    # Raw consumption.yaml (Phase 12 patterns + questionnaire C/P/F distribution).
    consumption = dict(raw["consumption.yaml"])

    _validate_cross_references(goals=goals, indicators=indicators, targets=targets, scoring=scoring)
    _validate_target_generator(target_generator, goals=goals)
    ...
    return NutritionEngineConfig(
        ...
        body_metric_targets=body_metric_targets,
...
def _parse_scoring(raw: dict[str, Any]) -> ScoringConfig:
    ...
    raw_tier_weights = raw.get("goal_alignment_tier_weights") or {}
    if not isinstance(raw_tier_weights, dict):
        raise ValueError("scoring.yaml: goal_alignment_tier_weights must be a mapping")
    critical_weight = float(raw_tier_weights.get("critical", 0.85))
    supporting_weight = float(raw_tier_weights.get("supporting", 0.15))
    if critical_weight < 0 or supporting_weight < 0:
        raise ValueError("scoring.yaml: goal alignment tier weights must be non-negative")
    if abs((critical_weight + supporting_weight) - 1.0) > 1e-6:
        raise ValueError("scoring.yaml: goal alignment tier weights must equal 1.0")
    ...
    return ScoringConfig(
        ...
        goal_critical_weight=critical_weight,
        goal_supporting_weight=supporting_weight,
...
def _parse_indicators(raw: dict[str, Any]) -> dict[str, IndicatorDefinition]:
    ...
    for indicator_id, payload in section.items():
        ...
        quality_group_codes = tuple(
            str(x) for x in (payload.get("quality_group_codes") or [])
        )
        protein_supporting_group_codes = tuple(
            str(x) for x in (payload.get("protein_supporting_group_codes") or [])
        )
        ...
        indicators[str(indicator_id)] = IndicatorDefinition(
            ...
            quality_group_codes=quality_group_codes,
            protein_supporting_group_codes=protein_supporting_group_codes,
        )
...
def _parse_macro_target_set(raw: Any, *, context: str) -> MacroTargetSet:
    if not isinstance(raw, dict):
        raise ValueError(f"targets.yaml: {context} must be a mapping")
    return MacroTargetSet(
        carbohydrate_percent=_parse_target_range(raw.get("carbohydrate_percent")),
        fat_percent=_parse_target_range(raw.get("fat_percent")),
        protein_percent=_parse_target_range(raw.get("protein_percent")),
        fibre_g=_parse_target_range(raw.get("fibre_g")),
    )


def _parse_target_generator(
    raw: dict[str, Any],
    *,
    targets: dict[str, TargetDefinition],
) -> TargetGeneratorConfig:
    goal_raw = raw.get("goal_macro_targets") or {}
    if not isinstance(goal_raw, dict):
        raise ValueError("targets.yaml: goal_macro_targets must be a mapping")
    goal_macros: dict[GoalId, MacroTargetSet] = {}
    for goal_id, payload in goal_raw.items():
        typed = _require_goal_id(goal_id, context="goal_macro_targets")
        goal_macros[typed] = _parse_macro_target_set(payload, context=f"goal_macro_targets.{typed}")

    general = _parse_macro_target_set(
        raw.get("general_macro_targets") or {},
        context="general_macro_targets",
    )

    water_raw = raw.get("water_targets") or {}
    if not isinstance(water_raw, dict):
        raise ValueError("targets.yaml: water_targets must be a mapping")
    water_targets: dict[str, dict[str, TargetRange]] = {}
    for priority, gender_map in water_raw.items():
        if not isinstance(gender_map, dict):
            raise ValueError(f"targets.yaml: water_targets.{priority} must be a mapping")
        parsed_gender: dict[str, TargetRange] = {}
        for gender_key, range_raw in gender_map.items():
            parsed = _parse_target_range(range_raw)
            if parsed is None:
                raise ValueError(f"targets.yaml: water_targets.{priority}.{gender_key} invalid")
            parsed_gender[str(gender_key)] = parsed
        water_targets[str(priority)] = parsed_gender

    uplift_raw = raw.get("activity_water_uplift_l") or {}
    if not isinstance(uplift_raw, dict):
        raise ValueError("targets.yaml: activity_water_uplift_l must be a mapping")
    uplift = {str(k): float(v) for k, v in uplift_raw.items()}

    macro_combo_raw = raw.get("macro_combination") or {}
    if not isinstance(macro_combo_raw, dict):
        raise ValueError("targets.yaml: macro_combination must be a mapping")
    macro_combination = {str(k): str(v) for k, v in macro_combo_raw.items()}

    zero_keys_raw = raw.get("zero_goal_target_keys") or []
    if not isinstance(zero_keys_raw, list):
        raise ValueError("targets.yaml: zero_goal_target_keys must be a list")
    zero_keys = tuple(str(x) for x in zero_keys_raw)
    for key in zero_keys:
        if key not in targets:
            raise ValueError(f"targets.yaml: zero_goal_target_keys unknown key {key!r}")

    return TargetGeneratorConfig(
        goal_macro_targets=goal_macros,
        general_macro_targets=general,
        water_targets=water_targets,
        activity_water_uplift_l=uplift,
        macro_combination=macro_combination,
        zero_goal_target_keys=zero_keys,
    )


def _parse_gender_ranges(raw: Any, *, context: str) -> dict[str, TargetRange]:
    if not isinstance(raw, dict):
        raise ValueError(f"targets.yaml: {context} must be a mapping")
    result: dict[str, TargetRange] = {}
    for gender, range_raw in raw.items():
        parsed = _parse_target_range(range_raw)
        if parsed is None:
            raise ValueError(f"targets.yaml: {context}.{gender} must be a target range")
        result[str(gender)] = parsed
    return result


def _parse_goal_metric_overrides(
    raw: Any,
    *,
    context: str,
) -> dict[GoalId, dict[str, TargetRange]]:
    if raw is None:
        return {}
    if not isinstance(raw, dict):
        raise ValueError(f"targets.yaml: {context} must be a mapping")
    result: dict[GoalId, dict[str, TargetRange]] = {}
    for goal_key, gender_ranges in raw.items():
        goal_id = _require_goal_id(goal_key, context=context)
        result[goal_id] = _parse_gender_ranges(
            gender_ranges,
            context=f"{context}.{goal_id}",
        )
    return result


def _parse_body_metric_targets(raw: dict[str, Any]) -> BodyMetricTargetConfig:
    section = raw.get("body_metric_targets")
    if not isinstance(section, dict):
        raise ValueError("targets.yaml: body_metric_targets must be a mapping")

    bmr_raw = ((section.get("bmr") or {}).get("reference_ranges"))
    if not isinstance(bmr_raw, dict):
        raise ValueError(
            "targets.yaml: body_metric_targets.bmr.reference_ranges must be a mapping"
        )
    bmr_ranges = {
        str(age_band): _parse_gender_ranges(
            gender_ranges,
            context=f"body_metric_targets.bmr.reference_ranges.{age_band}",
        )
        for age_band, gender_ranges in bmr_raw.items()
    }

    waist_raw = section.get("waist_cm") or {}
    body_fat_raw = section.get("body_fat_percent") or {}
    if not isinstance(waist_raw, dict) or not isinstance(body_fat_raw, dict):
        raise ValueError("targets.yaml: waist_cm and body_fat_percent must be mappings")

    return BodyMetricTargetConfig(
        bmr_reference_ranges=bmr_ranges,
        waist_general=_parse_gender_ranges(
            waist_raw.get("general"),
            context="body_metric_targets.waist_cm.general",
        ),
        waist_goal_overrides=_parse_goal_metric_overrides(
            waist_raw.get("goal_overrides"),
            context="body_metric_targets.waist_cm.goal_overrides",
        ),
        body_fat_general=_parse_gender_ranges(
            body_fat_raw.get("general"),
            context="body_metric_targets.body_fat_percent.general",
        ),
        body_fat_goal_overrides=_parse_goal_metric_overrides(
            body_fat_raw.get("goal_overrides"),
            context="body_metric_targets.body_fat_percent.goal_overrides",
        ),
    )


def _validate_target_generator(
    target_generator: TargetGeneratorConfig,
    *,
    goals: dict[GoalId, GoalProfile],
) -> None:
    for goal_id in goals:
        if goal_id not in target_generator.goal_macro_targets:
            raise ValueError(
                f"targets.yaml: goal_macro_targets missing Healthy macros for {goal_id!r}"
            )
